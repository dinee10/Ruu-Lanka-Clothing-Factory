...
function HomeContent_2() {
  ...
  useEffect(() => {
    ...
    axios.get('http://localhost:5174/product') // Replace with your actual API endpoint
      .then(response => {
        ...
        <Row gutter={[16, 16]}>
          {products.map((products) => (
            <Col xs={24} sm={12} md={8} lg={6} key={products._id}> {/* 4 columns on large screens */}
              <Card
                hoverable
                cover={<img alt={products.name} src={`http://localhost:5174/${products.image}`} />} // product.image should be the image URL
